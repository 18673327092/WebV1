...
function Seach() {
    ...
    var grid = $("#grid").kendoTreeList({
        ...
        id: "MenuID",
        columns: [
            { field: "MenuName", expandable: true, title: "菜单名称", width: 250 },
            {
                width: 60,
            ...
            {
                width: 90,
                headerTemplate: "<label class='form-tag checkbox-inline' ><input  onClick='chkAll(this)'  style='margin-left: -15px; margin-top: 1px;' data-type='chk_dele' type='checkbox'/>彻底删除</label>",
                headerAttributes: { style: "text-align:center" },
                attributes: { style: "text-align:center" },
                template: function (data) {
            ...
            },
            {
                ...
                template: function (data) {
                    ...
                    if (OperationJSON.length <= 0) return;
                    $.each(OperationJSON, function (m, n) {
                        check.push("<div style='float:left;'><label class='form-tag checkbox-inline'><input  name='chk' " + (data.OperationCheck.indexOf(+n.ID + ",") != -1 ? "checked" : "") + " data-menuid='" + data.MenuID + "' style='margin-left: -15px;' type='checkbox' value='" + n.ID + "'>" + n.OperateName + "</label></div>");
                    })
                    return check.join("");
                }
            }
...
function Save() {
    ...
    $.each($("#grid").find("tbody").find("tr"), function (i, l) {
        ...
        var obj = {
            ...
            MenuID: $(l).find("td").find("input[name=chk]:eq(0)").data("menuid"),
        }
        var op = [];
