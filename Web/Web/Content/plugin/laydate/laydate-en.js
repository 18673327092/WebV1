...
function (a) {
    ...
    c.check = function () {
        ...
        if ("" !== d.replace(/\s/g, "")) {
            ...
            c.msg("The date does not conform to the format. Please reselect it."),
            1;
            if (f[0]) return c.elem[h.elemv] = "",
            c.msg("The date is not within the validity period, please select again."),
            1;
    ...
    c.viewDate = function (a, b, d) {
        ...
        c.ymd = f.ymd,
        h.year.value = c.ymd[0] + "Year",
        h.month.value = c.digit(c.ymd[1] + 1) + "Month",
        c.each(h.mms,
        function (a, b) {
    ...
    c.viewYears = function (a) {
        ...
        c.each(new Array(14),
        function (b) {
            d += 7 === b ? "<li " + (parseInt(h.year.value) === a ? 'class="' + h[2] + '"' : "") + ' y="' + a + '">' + a + "Year</li>" : '<li y="' + (a - 7 + b) + '">' + (a - 7 + b) + "Year</li>"
        }),
    ...
    c.events = function () {
        ...
        e.hmss = ["Hour", "Minutes", "Seconds"],
        e.hmsarr = [],
        c.msg = function (a, d) {
            var f = '<div class="laydte_hsmtex">' + (d || "Prompt") + "<span>×</span></div>";
