    ...
    function setDatasourceProperty(wid, path, val) {
        ...
        while (i < path.length) {
            ...
            if (data instanceof kendo.data.ObservableObject) {
                ...
                var Sys_data = new Date(data);
                val = parseInt(val, 10);
                if (isNaN(val)) return false;
                switch (prop) {
                    case "year": Sys_data.setFullYear(val); break;
                    case "month": Sys_data.setMonth(val); break;
                    case "date": Sys_data.setDate(val); break;
                    case "hours": Sys_data.setHours(val); break;
                    case "minutes": Sys_data.setMinutes(val); break;
                    case "seconds": Sys_data.setSeconds(val); break;
                    case "milliseconds": Sys_data.setMilliseconds(val); break;
                    default:
                        return false;
                }
                setDatasourceProperty(w, path.slice(0, -1), Sys_data);
                return Sys_data;
            }
    ...
    function displayJSON(orig_obj, options) {
        ...
        var html = function rec(obj) {
            ...
            function displayLazyProp(name, value, x) {
                ...
                html += wrapProperty(name, value) + ": ";
                html += json_heading(x.title);
                html += "<div class='codefold-body'>鈥�</div></div>";
                return html;
            }
