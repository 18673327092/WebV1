    ...
    public class WxPayApi
    {
        ...
        */
        public static WxPayData Micropay(WxPayData inputObj, int timeOut = 10)
        {
            ...
            inputObj.SetValue("spbill_create_ip", Wechat_Config.IP);//终端ip
            inputObj.SetValue("appid", Wechat_Config.APPID);//公众账号ID
            inputObj.SetValue("mch_id", Wechat_Config.MCHID);//商户号
        ...
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            {
                inputObj.SetValue("notify_url", Wechat_Config.NOTIFY_URL);//异步通知url
            }
	    ...
	    */
	    ...
	    {
		    //如果不需要进行上报
		    if(Wechat_Config.REPORT_LEVENL == 0)
            {
			    return;
		    } 

		    //如果仅失败上报
		    if(Wechat_Config.REPORT_LEVENL == 1 && inputObj.IsSet("return_code") && inputObj.GetValue("return_code").ToString() == "SUCCESS" &&
	    ...
	    */
	    public static WxPayData Report(WxPayData inputObj, int timeOut = 1)
	    {
		    ...
		    inputObj.SetValue("appid", Wechat_Config.APPID);//公众账号ID
		    inputObj.SetValue("mch_id", Wechat_Config.MCHID);//商户号
		    inputObj.SetValue("user_ip",Wechat_Config.IP);//终端ip
