...
namespace Web.Utility.Captcha
{
    ...
    public class CaptchaImage
    {
        ...
        {
            var captch = new CaptchaImage() { Height = 32, Width = 80 };
            var verifycode = new HttpCookie("verifycode");
            var text = string.Format("{0}{1}", guid, captch.Text.Trim());
            verifycode.Value = text.ToLower().EncryptOneWay<System.Security.Cryptography.SHA256CryptoServiceProvider>().ToLower();
            HttpContext.Current.Response.Cookies.Remove("verifycode");
